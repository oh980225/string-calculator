...
public class Parser {
  ...
  void parse(Character c, Run run) { // stack들만 넘겨주면 또 상태가 변하는 지점이 숨게 된다..
    if (isSupportedOperator(c)) { // 파싱
      run.pushOperator(valueOf(c));
      return;
    }

    if (c == ' ' && run.isPresentNumberPiece()) { // 파싱
      run.pushNumber(run.takeOutNumber());
      return;
    }

    if ('0' <= c && c <= '9') {
      run.pushNumberPiece(c);
    }
  }
}
